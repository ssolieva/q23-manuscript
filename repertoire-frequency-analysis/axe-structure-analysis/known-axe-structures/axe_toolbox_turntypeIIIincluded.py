...
def generate_dssp(pdb_path, cdrh3_sequence, cdrh3_index):
    '''
...
def generate_cdrh3_phi_angles(pdb_path, cdrh3_sequence, cdrh3_index):
    '''
...
def generate_cdrh3_psi_angles(pdb_path, cdrh3_sequence, cdrh3_index):
    '''
...
def generate_hbonds(pdb_path, cdrh3_sequence, name, cdrh3_index):
    '''
...
def generate_distances(pdb_path, cdrh3_sequence, name, cdrh3_index):
    '''
...
def run_all_commands(name, pdb_path, cdrh3_sequence):
    import mdtraj as md
    import numpy as np
    import matplotlib.pyplot as plt
    from math import pi
    ...
    dssp_values      = generate_dssp(pdb_path, cdrh3_sequence, cdrh3_index)
    phi_angles       = generate_cdrh3_phi_angles(pdb_path, cdrh3_sequence, cdrh3_index)*180/pi
    psi_angles       = generate_cdrh3_psi_angles(pdb_path, cdrh3_sequence, cdrh3_index)*180/pi
    hbonds3, hbonds4 = generate_hbonds(pdb_path, cdrh3_sequence, name, cdrh3_index)
    distances        = generate_distances(pdb_path, cdrh3_sequence, name, cdrh3_index)
    
    return phi_angles, psi_angles, dssp_values, hbonds3, hbonds4, distances 

# Step 9: create a dataframe:

def generate_dataframe(features,name):
    import pandas as pd
    ResidueIndex = []
    ResidueType  = []
    Phi = []
...
def check_for_pattern(combined_data, distances, beta_sheet_distance_cutoff):
    ...
    turn_resis = []
    beta_resis = []
    for i in range(len(combined_TB_joined)-1):
...
def plot_turn_beta_distance(csv_file, name, combined_TB, beta_sheet_distance_cutoff):
    classification_of_hh = check_for_pattern_v2(combined_TB, csv_file['distances'], beta_sheet_distance_cutoff)
    scatter_beta = []
    scatter_turn = []
    scatter_neither = []
    scatter_all = []
    ...
    plt.tight_layout()
    plt.savefig(f'plots/{name}_combined_turn_beta_distance_turntypeIIIincluded.png')
    plt.close()
...
# check if it is a hammerhead or not according to the turn-beta-turn pattern:
def check_for_pattern_v2(combined_data, distances, beta_sheet_distance_cutoff):
    '''Newest version, added on 07172024 '''
    combined_TB_joined = ''.join(combined_data)
    turn_resis = []
    beta_resis = []
    for i in range(len(combined_TB_joined)-2):
        if combined_TB_joined[i] in 'TFZ' and combined_TB_joined[i+1] in 'TFZ':
            turn_resis.append(i)
        if combined_TB_joined[i] in 'DF' and combined_TB_joined[i+1] in 'DF' and combined_TB_joined[i+2] in 'DF':
            beta_resis.append(i) # either far beta or far beta+turn
    # print(turn_resis, beta_resis)
    # non consecutive:
    from itertools import groupby
...
def classify_turn_beta_distance_v2(csv_file, name, combined_TB, beta_sheet_distance_cutoff):
    classification_of_hh = check_for_pattern_v2(combined_TB, csv_file['distances'], beta_sheet_distance_cutoff)
    return classification_of_hh



def plot_turn_beta_distance_known(csv_file, name, combined_TB, beta_sheet_distance_cutoff, turn1x1, turn2x2):
    classification_of_hh = check_for_pattern_v2(combined_TB, csv_file['distances'], beta_sheet_distance_cutoff)
    scatter_beta = []
    scatter_turn = []
    scatter_neither = []
    ...
    plt.yticks([0,1,2], ['Other', 'Beta', 'Turn'], fontsize=16, weight='bold')
    plt.xticks(np.arange(len(csv_file['ResidueType'])), csv_file['ResidueType'], fontsize=15, weight='bold')
    plt.ylim(-0.5,2.5)
    plt.axvspan(0-0.5, turn1x1+0.5-1,                            facecolor='dimgray', alpha=0.3)
    plt.axvspan(turn2x2-0.5+1, len(csv_file['ResidueType'])+0.5, facecolor='dimgray', alpha=0.3)

    plt.scatter(np.arange(len(csv_file['ResidueType'])), scatter_turn,    c='blue',   edgecolor='k',    s=200, alpha=1, lw=3)
    plt.scatter(np.arange(len(csv_file['ResidueType'])), scatter_beta,    c=beta_colors, edgecolor='k', s=200, alpha=1, lw=3)
    plt.scatter(np.arange(len(csv_file['ResidueType'])), scatter_neither, c='gray',   edgecolor='k',    s=200, alpha=1, lw=3)
    plt.xlim(-0.5,len(csv_file['ResidueType'])-0.5)

    plt.tight_layout()
    plt.savefig(f'plots/{name}_combined_turn_beta_distance_turntypeIIIincluded.pdf', dpi=300, transparent=True)
    plt.close()
